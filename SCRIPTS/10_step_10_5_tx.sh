#!/usr/bin/env bash
# -------------------------------------------------------------------------
# CLIMB workflow step: 10  (Gap-filling daily maximum temperature with ERA5 near coast)
# Purpose:      Use ERA5-based daily maximum temperature (tx, from tasmax/tn)
...
#
# Inputs:       - BASD+SD tx for 1990–2022 (from Step 9, in °C):
#                   ${output_dir}/step_9/tx_YYYY.nc
#               - ERA5-based tasmax (affected by earlier swap issue) from Step 2:
#                   ${output_dir}/step_2/ERA5_for_gapfill/tasmax_ERA5_YYYY.nc
#
# Outputs:      - Gap-filled EMO-1-style tx files:
#                   ${output_dir}/step_10/tx_YYYY.nc
#
...
basd_file="${ipath}tx_1990.nc"  # from step 9
...
era5_file="${epath}tasmax_ERA5_1990.nc"  # from step 2
...
for y in {1990..2022}
do
    filename="${ipath}tx_${y}.nc"  # BASD+SD tx from step 9 (°C)
    tile="$(echo "$(basename "$filename" .nc)" | cut -d'_' -f2)"

    era5_file="${epath}tasmax_ERA5_${tile}.nc"         # ERA5 tasmax file from step 2
    ...
    # Note:
    # Added -expr,tx="tn-273.15" to fix an earlier issue in Step 2 where tx and tn
    # were swapped. Here we reconstruct tx in °C from the tn variable (in K)
    # contained in the tasmax-based file.
    cdo -f nc4c -z zip remap,"$grid_file","$weight_file" -expr,tx="tn-273.15" "$era5_file" "$era5_file_regrid"

    # Use BASD mask to select BASD tx values over land and ERA5 tx over remaining cells
