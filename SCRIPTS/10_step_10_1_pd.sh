#!/usr/bin/env bash
# -------------------------------------------------------------------------
# CLIMB workflow step: 10  (Gap-filling vapour pressure with ERA5 near coast)
# Purpose:      Use ERA5-based vapour pressure (pd) to fill coastal/masked
...
#
# Inputs:       - BASD+SD vapour pressure for 1990–2022 (from Step 9):
#                   ${output_dir}/step_9/pd_YYYY.nc
#               - ERA5-based humidity and temperature (from Step 2):
#                   ${output_dir}/step_2/ERA5_for_gapfill/hurs_ERA5_YYYY.nc
#                   ${output_dir}/step_2/ERA5_for_gapfill/tas_ERA5_YYYY.nc
#                   ${output_dir}/step_2/ERA5_for_gapfill/pd_ERA5_YYYY.nc
#
# Outputs:      - Gap-filled EMO-1-style vapour pressure files:
#                   ${output_dir}/step_10/pd_YYYY.nc
#
...
basd_file="${ipath}pd_1990.nc"  # from step 9
...
era5_file="${epath}hurs_ERA5_1990.nc"  # from step 2
...
for y in {1990..2022}
do
    filename="${ipath}pd_${y}.nc"  # BASD+SD pd from step 9
    tile="$(echo "$(basename "$filename" .nc)" | cut -d'_' -f2)"

    era5_file="${epath}hurs_ERA5_${tile}.nc"   # ERA5 relative humidity from step 2
    ...
    # Calculate ERA5-based vapour pressure (pd) from relative humidity and dew point
    ...
    cdo -f nc4c -z zip expr,pd="hurs / 100 * (6.11 * 10 ^ (7.5 * (dpt - 273.15) / (237.3 + (dpt - 273.15) ) ))" \
        -merge "$era5_file" "$era5_file_tas" "$era5_file_pd"

    # Remap ERA5 vapour pressure to the BASD grid and shift time by +1 day
