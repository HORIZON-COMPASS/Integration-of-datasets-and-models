#!/usr/bin/env bash
# -------------------------------------------------------------------------
# CLIMB workflow step: 10  (Gap-filling shortwave radiation with ERA5 near coast)
# Purpose:      Use ERA5-based shortwave radiation (rsds) to fill coastal/masked
...
#
# Inputs:       - BASD+SD radiation for 1990–2022 (from Step 9):
#                   ${output_dir}/step_9/rg_YYYY.nc
#               - ERA5-based shortwave radiation (from Step 2):
#                   ${output_dir}/step_2/ERA5_for_gapfill/rsds_ERA5_YYYY.nc
#
# Outputs:      - Gap-filled EMO-1-style radiation files:
#                   ${output_dir}/step_10/rg_YYYY.nc
#
...
basd_file="${ipath}rg_1990.nc"  # from step 9
...
era5_file="${epath}rsds_ERA5_1990.nc"  # from step 2
...
for y in {1990..2022}
do
    filename="${ipath}rg_${y}.nc"  # BASD+SD rg from step 9
    tile="$(echo "$(basename "$filename" .nc)" | cut -d'_' -f2)"

    era5_file="${epath}rsds_ERA5_${tile}.nc"        # ERA5 shortwave radiation from step 2 (W m-2)
    ...
    # Convert ERA5 rsds (W m-2) to daily rg (J m-2 day-1) for consistency with EMO-1
    # and remap to the BASD grid, shifting time by +1 day to match EMO-1 convention.
    cdo -f nc4c -z zip remap,"$grid_file","$weight_file" -expr,rg="rsds*86400" -shifttime,1days "$era5_file" "$era5_file_regrid"
    ...
    # Remove temporary files for this year (optional, can be used to troubleshoot issues)
    rm t*_rg_*.nc
done
